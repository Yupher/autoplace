...
exports.addAdmin = catchAsync(async (req, res, next) => {
    ...
    if (!req.body.email && !req.body.number) {
        return next(
            new AppError(
                'Please, Give us all informations.',
                404
            )
        );
    }
    ...
    if (!current) {
        return next(
            new AppError(
                'We cannot find this user, please give us a valid ID.',
                404
            )
        );
    }
    ...
    if (req.body.email) {
        ...
        user = await User.findOneAndUpdate({ number: req.body.number }, { role: req.body.role }, {
            new: true,
            runValidators: true
        });
    }
...
exports.deleteAdmin = catchAsync(async (req, res, next) => {
    ...
    const user = await User.findByIdAndUpdate(req.body.user, { role: current.isFirst ? current.isFirst : 'client' }, {
        new: true,
        runValidators: true
    });
...
exports.updateMe = catchAsync(async (req, res, next) => {
    ...
    if (req.body && (req.body.role || req.body._id || req.body.password || req.body.passwordConfirm)) {
        return next(
            new AppError(
                'Do not redo this operation, you risked deleting your account immediately.',
                404
...
exports.updatePassword = catchAsync(async (req, res, next) => {
    ...
    if (!password || !passwordConfirm || !passwordOld || password !== passwordConfirm) {
        return next(
            new AppError(
                'Please, give us a correct password.',
                404
            )
        );
    }
    ...
    const updatedUser = await User.findByIdAndUpdate(req.user.id, { password, passwordConfirm });
